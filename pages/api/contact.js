...
export default async function handler(req, res) {
	...
	if (honeypot && honeypot.trim() !== '') {
		console.log(`[Contact] Honeypot triggered for IP: ${ip}`);
	...
	if (submissionTime && submissionTime < 3000) {
		console.log(
			`[Contact] Submission too fast (${submissionTime}ms) for IP: ${ip}`
		);
		return res.status(400).json({
	...
	try {
		...
		if (!recaptchaData.success) {
			console.log(
				`[Contact] reCAPTCHA verification failed for IP: ${ip}`,
				recaptchaData
			);
			return res.status(400).json({
